...
typedef struct FreeBolck
{
    int index;
    struct FreeBolck *prev;
    struct FreeBolck *next;
...
FileNode *root, *cwd;
FreeBolck *freeListHead = NULL, *freeListTail = NULL;
...
{
    FreeBolck *block = (FreeBolck *)malloc(sizeof(FreeBolck));
    block->index = index;
    block->next = NULL;
...
void mkdirCmd(char *name) {
    ...
    newDir->child = NULL;
    newDir->next = NULL;
    for (int i = 0; i < MAXIMUM_BLOCKS_PER_FILE; i++) newDir->blockPointers[i] = -1;
...
void createcmd(char *name){
    ...
    for (int i = 0; i < MAXIMUM_BLOCKS_PER_FILE; i++) newFile->blockPointers[i] = -1;
