...
int* jimOrders(int orders_rows, int orders_columns, int** orders, int* result_count) {
    ...
    int *customer = malloc(orders_rows * sizeof(int));
    int *servetime = malloc(orders_rows * sizeof(int));
    ...
    for(int i=0;i<orders_rows-1;i++){
        for(int j=0;j<orders_rows-1-i;j++){
            if(*(servetime+j)>*(servetime+j+1)){
                //sort serving time
               int temp= *(servetime+j);
                *(servetime+j) = *(servetime+j+1);
                *(servetime+j+1) = temp;
                
                //sorting customers on the basis of serve time
